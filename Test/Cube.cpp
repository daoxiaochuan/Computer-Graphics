...
void CCube::ReadFace(void)//面表
{
	...
	F[2].ptNumber = 4, F[2].ptIndex[0] = 0, F[2].ptIndex[1] = 4, F[2].ptIndex[2] = 7, F[2].ptIndex[3] = 3, F[2].c = CRGB(0.0, 0.0, 1.0);//左面蓝
...
void CCube::Draw(CDC* pDC ,CZBuffer* pZBuffer,int nSign)
{
	CP3 ScreenPoint[4];//二维投影点
	for (int nFace = 0; nFace < 6; nFace++)//面循环
	{
		for (int nPoint = 0; nPoint < F[nFace].ptNumber; nPoint++)//顶点循环
		{
			ScreenPoint[nPoint] = projection.PerspectiveProjection3(V[F[nFace].ptIndex[nPoint]]);//透视投影
			if (nSign == 0)
